...
def plot_KL(data):
    ...
    lengths = [min(Ncat, len(mc)) for mc in data.fiducial]  # Cut to the minimum Ncat
...
def plot_loglog(data):
    ...
    ax.plot(data.cdf[0], Ncat*data.cdf[1],
            ls='--', color='Magenta', linewidth=3, label='Tree distr.')

    ax.plot(counts, Ncat*data.lognorm_fit(),
    ...
    avg = helpers.avg_mc(data.fiducial, data.counts)
    Nc = len(avg)
    ...
    # plt.savefig(os.path.join('all_data', 'new_loglog_'+data.name+'.png'))
    return
...
def plot_loglog(data):
    ...
    # plt.savefig(os.path.join('all_data', 'new_loglin_'+data.name+'.png'))
    return


def main():
    names = ('amazon', 'amazon_new', 'arxiv', 'bacteria', 'cath', 'counties',
             'files', 'germanpop', 'harvard', 'hospital', 'merchants',
             'microbiome', 'mta', 'NATC', 'quora',
             'pop2014', 'raul', 'reddit', 'swisspop')
    names = ('arxiv2',)
    sigma_Ncat = np.empty((len(names), 4))
    #sigma_Ncat = pickle.load(open('sigma_Ncat.pkl', 'rb'))

    for i, name in enumerate(names):
        print('\nDoing '+name+'.....')
        data = Dataset(os.path.join(dirdata, name+'.pkl'))
        ...
        print('Our estimated Q: ', data.factor * data.Ncat)
        print('Ratio Q/Chao: ', data.factor * data.Ncat/chao_est)
